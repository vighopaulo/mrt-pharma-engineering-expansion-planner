    ...
    with st.expander("View full side-by-side implementation plan",expanded=False):
        ...
        def details_df(c,is_mrt):
            rows={
                ...
                "Additional injection rooms":c.R_a if is_mrt else c.injection_additional,
                "Additional uptake rooms":c.R_a if is_mrt else c.uptake_additional,
            ...
            return pd.DataFrame([{"Metric":k,"Value":v} for k,v in rows.items()])
        with dc1:
