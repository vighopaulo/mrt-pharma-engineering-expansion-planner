...
with st.form("digital_twin_form"):
    ...
    st.markdown(
        "### 1. Clinical Demand and Distributed Oncology"
    )
    ...
    with a4:
        ...
        maximum_clinical_nodes = st.number_input(
            "Maximum clinical access points",
            min_value=1,
            max_value=100,
            value=8,
            step=1,
            help=(
                "Examples include pediatric wings, adult oncology "
                "wings, outpatient centers, injection suites, and "
                "inpatient oncology service points."
            ),
        )
    ...
    st.markdown(
        "### 2. Injection, Uptake, FDG Production, and Transport"
    )
    ...
    st.markdown(
        ...
        <div class="assumption-note">
            ...
            The MRT workflow uplift is explicitly displayed and editable.
            It represents potential reductions in transport delays,
            manual handoffs, workflow interruptions, and scanner idle time.
            It should be validated using hospital-specific operational data.
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    with u1:
        distribution_uplift_per_node_pct = st.number_input(
            "MRT workflow uplift/access point (%)",
            min_value=0.0,
            max_value=20.0,
            value=2.0,
            step=0.5,
    ...
    with u3:
        baseline_distribution_efficiency_pct = st.number_input(
            "Initial distributed efficiency (%)",
            min_value=1.0,
            max_value=100.0,
            value=82.0,
            step=1.0,
    ...
    st.markdown(
        "### 3. CapEx, OpEx, Revenue, and Financial Assumptions"
    )
...
if submitted:
    ...
    # No hidden scenario multipliers are used.
    # Every displayed value is used directly.

    effective_availability = scanner_availability
    effective_synthesis_yield = synthesis_yield
    effective_qc_yield = qc_yield
    effective_revenue = revenue_per_incremental_patient
    ...
    # ========================================================
    # BEST CONFIGURATION FOR EACH ARCHITECTURE
    # ========================================================
    ...
    def render_card(
        ...
        if candidate:
            ...
            <div class="{card_css}">
                ...
                <div>
                    <b>Throughput:</b>
                    {candidate['throughput']:.0f}/day
                </div>

                <div>
                    <b>Incremental patients:</b>
                    {candidate['incremental_patients_day']:.0f}/day
                </div>
                ...
                <div>
                    <b>Annual OpEx:</b>
                    ${candidate['annual_opex']:,.0f}
                </div>

                <div>
                    <b>Upgrade:</b>
                    {candidate['upgrade_pct']}%
                </div>

                <div>
                    <b>Additional scanners:</b>
                    {candidate['add_scanners']}
                </div>

                <div>
                    <b>Injection rooms added:</b>
                    {candidate['add_injection']}
                </div>

                <div>
                    <b>Uptake rooms added:</b>
                    {candidate['add_uptake']}
                </div>

                <div>
                    <b>Additional batches/day:</b>
                    {candidate['added_batches']}
                </div>

                <div>
                    <b>Clinical access points:</b>
                    {candidate['clinical_nodes']}
                </div>

                <div>
                    <b>Payback:</b>
                    {candidate['payback_years']:.1f} years
                </div>

                <div>
                    <b>{analysis_years}-year ROI:</b>
                    {candidate['roi_pct']:.1f}%
                </div>
            ...
            <div class="{card_css}">
                ...
                <div>
                    No configuration met the selected
                    physical, throughput, budget, and
                    positive-cash-flow constraints.
                </div>
            </div>
            """
    ...
    if decision:
        ...
        reasons = [
            (
                ...
                f"{target_patients} PET patients per day."
            ),
            (
            ...
            (
                ...
                f"upgrade and {decision['added_batches']} additional "
                f"batches per day."
            ),
            (
                f"Preserves "
        ...
        [
            {
                ...
                "Annual Incremental Revenue (USD)": (
                    candidate[
                        "annual_incremental_revenue"
                    ]
                ),
                "Annual Net Cash Flow (USD)": (
                    candidate["annual_net"]
                ),
                "Upgrade (%)": (
                    candidate["upgrade_pct"]
                ...
                "Annual OpEx/Incremental Patient (USD)": (
                    candidate[
                        "cost_per_incremental_patient"
                    ]
                ),
    ...
    assumptions_df = pd.DataFrame(
        [
            {
                "Assumption": (
                    "Modeled baseline throughput/day"
                ),
                "Value": current_throughput,
            },
            {
                "Assumption": (
                    "Target throughput/day"
                ),
            ...
            {
                "Assumption": (
                    "Baseline distributed efficiency (%)"
                ),
                "Value": (
                    baseline_distribution_efficiency_pct
                ),
    ...
    if not results_df.empty:
        ...
        st.warning(
            ...
            "Review the demand, capacity, revenue, cost, "
            "production, and budget assumptions."
        )

    st.markdown(
        """
        <div class="footer">
            MRT Pharma™ Digital Twin demonstration model. Outputs are
            illustrative and depend on user-entered assumptions. The model
            evaluates 18F-FDG PET imaging, injection and uptake capacity,
            transport capacity, distributed clinical access points, CapEx,
            OpEx, incremental revenue, ROI, NPV, and discounted break-even.
            It is not a substitute for hospital engineering, clinical,
            financial, or regulatory validation.
        </div>
        """,
        unsafe_allow_html=True,
    )
