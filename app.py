...
st.markdown(
    ...
    .winner-card {
        background: linear-gradient(135deg, #effff3, #dff7e7);
        border: 2px solid #2e9d50;
        border-top: 8px solid #2e9d50;
        box-shadow: 0 10px 28px rgba(46, 157, 80, 0.18);
    }
...
def centralized_intake_points(total_scanners: int) -> int:
    """One centralized intake point per two scanners, rounded upward."""
    return max(1, math.ceil(total_scanners / 2))
...
with st.form("digital_twin_form"):
    ...
    with a4:
        ...
        maximum_clinical_nodes = st.number_input(
            "Maximum MRT-connected clinical delivery points",
            min_value=1,
            max_value=500,
            value=50,
            step=1,
            help=(
                "Each approved inpatient room, injection room, treatment room, "
                "or other last-mile clinical destination that can receive a "
                "radiopharmaceutical through MRT Pharma counts as one point."
            ),
        )
    ...
    st.markdown(
        ...
        <div class="assumption-note">
            <b>MRT inpatient-room assumption:</b>
            The hybrid architecture may use suitably prepared inpatient oncology
            rooms as distributed administration and uptake locations. Existing
            dedicated outpatient injection and uptake rooms remain available.
            The hybrid search does not automatically construct new dedicated
            injection or uptake rooms.
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    with r1:
        total_oncology_room_inventory = st.number_input(
            "Total rooms allocated to oncology functions",
            min_value=1,
            max_value=1000,
            value=500,
            step=1,
            help=(
                "Physical-room ceiling for dedicated injection rooms, "
                "dedicated uptake rooms, and MRT-enabled inpatient rooms. "
                "Each physical room is counted once."
            ),
        )

    with r2:
        shared_injection_uptake_rooms = st.number_input(
            "Injection/uptake rooms that are physically shared",
            min_value=0,
            max_value=100,
            value=0,
            step=1,
            help=(
                "Enter the number of rooms already counted in both the "
                "injection-room and uptake-room totals. This removes physical "
                "double-counting without reducing their functional capacity."
            ),
        )

    with r3:
        available_inpatient_oncology_rooms = st.number_input(
            "Available inpatient oncology rooms",
            min_value=0,
            max_value=1000,
            value=170,
            step=1,
    ...
    with r4:
        max_mrt_enabled_inpatient_rooms = st.number_input(
            "Maximum MRT-enabled inpatient rooms",
            min_value=0,
            max_value=1000,
            value=170,
            step=1,
    ...
    with r5:
        patients_per_enabled_inpatient_room = st.number_input(
            "Patients supported/enabled inpatient room/day",
            min_value=0.1,
            max_value=20.0,
            value=1.0,
            step=0.1,
    ...
    with r6:
        inpatient_room_enablement_capex = st.number_input(
            "MRT enablement CapEx/inpatient room (USD)",
            min_value=0.0,
            ...
            format="%.0f",
            help=(
                "Set to zero for a purpose-built hospital where the room is "
                "already designed for the intended clinical use."
            ),
        )
    ...
    with h2:        annual_opex_per_enabled_inpatient_room = st.number_input(
            "Annual OpEx/MRT-enabled inpatient room (USD)",
            min_value=0.0,
            max_value=5_000_000.0,
            value=0.0,
            step=10_000.0,
...
if submitted:
        ...
        for add_injection in injection_values:
            for add_uptake in uptake_values:
                ...
                add_financial_candidate(
                    ...
                    add_injection,
                    add_uptake,
                            centralized_intake_points(current_scanners + add_scanners),
    ...
    # Existing dedicated rooms remain available for outbound patients.
    # MRT-enabled inpatient rooms add distributed administration and uptake capacity.
    # The hybrid does not automatically construct new dedicated injection/uptake rooms.
    # Physical room inventory:
    # Injection and uptake rooms may overlap physically. The overlap is
    # subtracted once for inventory purposes, but both functional capacities
    # remain available in the clinical-capacity equations below.
    valid_shared_rooms = min(
        shared_injection_uptake_rooms,
        current_injection_rooms,
        current_uptake_rooms,
    )

    unique_centralized_rooms = (
        current_injection_rooms
        + current_uptake_rooms
        - valid_shared_rooms
    )

    room_inventory_remaining_for_mrt = max(
        0,
        total_oncology_room_inventory - unique_centralized_rooms,
    )

    max_enabled_rooms = min(
        available_inpatient_oncology_rooms,
        max_mrt_enabled_inpatient_rooms,
        room_inventory_remaining_for_mrt,
        max(0, maximum_clinical_nodes - current_injection_rooms),
    )
    ...
    for upgrade_pct in upgrade_values:
        for added_batches in batch_values:
            ...
            for enabled_rooms in range(max_enabled_rooms + 1):
                delivery_points = current_injection_rooms + enabled_rooms

                if delivery_points < 1 or delivery_points > maximum_clinical_nodes:
                    continue
                ...
                distributed_room_capacity = (
                    enabled_rooms
                    * patients_per_enabled_inpatient_room
                )

                # Hybrid capacity includes BOTH:
                # 1. the ordinary centralized outpatient injection/uptake rooms;
                # 2. the MRT-enabled inpatient oncology rooms.
                #
                # The enabled inpatient room is counted once physically, but it
                # may support both administration and uptake for the same patient.
                centralized_injection_capacity = (
                    current_injection_rooms
                    * patients_per_injection_room
                )

                centralized_uptake_capacity = (
                    current_uptake_rooms
                    * patients_per_uptake_room
                )
                ...
                for add_scanners in scanner_values:
                    ...
                    capex = (
                        ...
                        * clinical_node_capex
                        + enabled_rooms
                        * inpatient_room_enablement_capex
                        + add_scanners
                        * scanner_capex
                    )
                    ...
                    annual_opex = (
                        ...
                        * annual_opex_per_scanner
                        + enabled_rooms
                        * annual_opex_per_enabled_inpatient_room
                    )
    ...
    title_by_architecture = {
        "New Cyclotron": "Option 1 — New Cyclotron",
        "Conventional Upgrade": "Option 2 — Conventional Upgrade",
        "MRT Pharma Hybrid": "Option 3 — MRT Pharma Hybrid",
    }
    ...
    def render_summary_card(
        ...
        is_winner = (
            ...
            and architecture_name == winner_architecture
        )
        ...
        if candidate is None:
            html = (
                ...
                f'{title_by_architecture[architecture_name]}</div>'
                f'<div class="big">NOT FEASIBLE</div>'
                f'<div class="summary-metric">'
                f'No feasible configuration met the selected constraints.'
                f'</div></div>'
            )
        else:
            winner_badge = (
            ...
            html = (
                ...
                f'<div class="card-title">'
                f'{title_by_architecture[architecture_name]}</div>'
                f'<div class="big">${candidate["capex"]:,.0f}</div>'
                f'{winner_badge}'
                f'<div class="summary-metric">'
                ...
                f'<div class="summary-metric">'
                f'<b>ROI:</b> {candidate["roi_pct"]:.1f}%</div>'
                f'</div>'
            )
    ...
    for column, architecture_name in zip(
        ...
        render_summary_card(
            column,
            architecture_name,
            best_by_architecture[architecture_name],
        )
    ...
    with st.expander(
        ...
        for column, architecture_name in zip(
            ...
            candidate = best_by_architecture[architecture_name]

            with column:
                st.markdown(
                    f"**{title_by_architecture[architecture_name]}**"
                )
                ...
                rows = [
                    ...
                    ("Cyclotron upgrade", f"{candidate['upgrade_pct']:.0f}%"),
                    ("Additional scanners", f"{candidate['add_scanners']:.0f}"),
                    ("Additional batches/day", f"{candidate['added_batches']:.0f}"),
                    ("Payback", f"{candidate['payback_years']:.1f} years"),
                    (f"{analysis_years}-year ROI", f"{candidate['roi_pct']:.1f}%"),
                    ("NPV", f"${candidate['npv']:,.0f}"),
                ]

                if architecture_name == "MRT Pharma Hybrid":
                    rows.extend(
                        [
                            (
                                "MRT-connected delivery points",
                                f"{candidate['clinical_nodes']:.0f}",
                            ),
                            (
                                "MRT-enabled inpatient rooms",
                                f"{candidate['enabled_inpatient_rooms']:.0f}",
                            ),
                            ("Dedicated injection rooms added", "0"),
                            ("Dedicated uptake rooms added", "0"),
                        ]
                    )
                else:
                    rows.extend(
                        [
                            (
                                "Centralized intake points",
                                f"{candidate['clinical_nodes']:.0f}",
                            ),
                            (
                                "Dedicated injection rooms added",
                                f"{candidate['add_injection']:.0f}",
                            ),
                            (
                                "Dedicated uptake rooms added",
                                f"{candidate['add_uptake']:.0f}",
                            ),
                            ("MRT-connected delivery points", "Not applicable"),
                        ]
                    )
    ...
    if decision:
        st.markdown(
            (
                ...
                '<div class="decision-title">Digital Twin Decision</div>'
                f'<div class="decision-name">'
                f'✓ {decision["architecture"]}</div>'
        ...
        st.warning(
            "No architecture met the selected physical, financial, "
    ...
    st.markdown(
        ...
        <div class="footer">
            ...
            capacity, transport capacity, centralized intake points, MRT-connected clinical delivery points,
