...
with st.form("digital_twin_form"):
    ...
    with h2:
        inpatient_room_enablement_capex = st.number_input(
            ...
            format="%.0f",
            help=(
                "Set to zero for a purpose-built hospital where rooms already "
                "meet the required clinical and engineering design."
            ),
        )
        annual_opex_per_enabled_inpatient_room = st.number_input(
    ...
    def render_card(
        ...
        if candidate:
            ...
            if candidate["architecture"] == "New Cyclotron":
                ...
                access_detail = (
                    f"<div><b>Centralized clinical intake points:</b> "
                    f"{candidate['clinical_nodes']}</div>"
                    "<div><b>MRT-connected delivery points:</b> Not applicable</div>"
                )
                ...
                enabled_rooms = max(
                    0,
                    candidate["clinical_nodes"] - current_injection_rooms,
                )
                access_detail = (
                    f"<div><b>MRT-connected clinical delivery points:</b> "
                    f"{candidate['clinical_nodes']}</div>"
                    f"<div><b>MRT-enabled inpatient rooms:</b> "
                    f"{enabled_rooms}</div>"
                )
            ...
            <div class="{css_class}">
                ...
                <div><b>Dedicated injection rooms added:</b> {candidate['add_injection']}</div>
                <div><b>Dedicated uptake rooms added:</b> {candidate['add_uptake']}</div>
